...
- 原寸ファイルを無圧縮コピー（一覧・ライトボックスともこのファイルを表示）
...
**画質ポリシー**：ギャラリーの表示はすべて**原寸ファイルをそのまま**使う。
縮小サムネの生成・lossy再エンコード・拡大表示はしない（画質＞ページの軽さ、がご主人の方針）。
...
│   └── kyown_○○.png      ← 原寸画像（一覧・ライトボックス共通で表示）
...
- ページに見出しなどのテキストは置かない（画像が主役）

### メイン画像（featured）

`gallery.json` の項目に `"featured": true` を付けると、その画像は
**グリッド全幅（メイン扱い）で先頭に大きく表示**される。現在は各キャラの
cover 画像（`○○_cover.png`）に付けてある。付け外ししたら `npm run gallery` で再生成する。

- featured もメイソンリー内の画像も、**すべて原寸ファイルをそのまま表示**する（画質優先。ページはその分重くなる）
